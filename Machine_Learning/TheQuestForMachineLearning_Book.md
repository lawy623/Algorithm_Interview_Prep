...
> 文本表示
    ...
    · TF-IFD=TF(t,d) * IDF(t)。IDF表示词的重要性，如果在多篇文章都出现过，说明词通用，并不重要。TF为词在文档d中的频率
...
> 统计分布
    ...
    · 伯努利分布：单次trail的分布，二项分布的特殊形式
    · 二项分布：独立进行n次0-1 trail的分布
    · 多项分布：进行n次对立trail，但每次的结果不是0-1，可以更多
    · 泊松分布：某段时间内发生时间次数的概率
    · 超几何分布：有放回的采样分布
    · 正态分布
    · 均匀分布
...
> boosting和bagging
    ...
    · boosting是串行的，对之前分错的类别后续会给予更高的权重->减小偏差
    · bagging是并行的，且模型之间没有太强的依赖（独立性）->减小方差（随机变量方差随着n增大而减小）
    
> adaboosting

    · 训练T个基分类器。最开始是随机采样样本进行训练
    · 通过对预测准确率的调整，调整采样率，对容易出错的样本给予更高的采样率
    · 基分类器的贡献权重跟错误率成反比
    · 通过加权求和获得最后的总体分类器
...
> gbdt
    ...
    · 每一颗树学习的是上一棵树带来的残差
    · 最后需要把所有结果加起来作为最终预测
    · 优点：解释性强；可并行 / 缺点：表达能力受限；训练时需要串行
    
> 决策树作为基分类器

    · 容易调整样本权重
    · 通过层数可以控制表达能力和泛化性
    · 数据扰动对模型影响大，容易让模型有不同的表达能力
    · 神经网络也有类似的特点