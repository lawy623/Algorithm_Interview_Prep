...
> Transformer常见面试问题
    ...
    （3）为什么在进行softmax之前需要对attention进行scaled（为什么除以dk的平方根），并使用公式推导进行讲解
     假设Q和K的均值为0，方差为1。它们的矩阵乘积将有均值为0，方差为dk，因此使用dk的平方根被用于缩放，因为，Q和K的矩阵乘积的均值本应该为0，方差本应该为1，这样可以获得更平缓的softmax。当维度很大时，点积结果会很大，会导致softmax的梯度很小。为了减轻这个影响，对点积进行缩放。
    ...
    （8）bert的mask为何不学习transformer在attention处进行屏蔽score的技巧？
...
> 方法
    ...
    · I3D：把3d conv做在rgb和flow的双流上。3Dconv与lstm方法相比，更好的建模了时序信息
    · TSN：整段视频拆分K段，每段取几帧，双流网络提特征，最后特征交叉求解。对双流网络的升级。但是与3D类方法相比时序信息不强
    ...
    · Vlad：将获取的N个特征做聚类，获得K的中心，累加每个中心的特征残差（相当于聚类内部距离中心的残差和）
    · NextVlad：特征融合的方法，通过fc+softmax做特征的加权。有点像senet/attention。但是softmax求解的时候不是全局
    · TRN：分成多个组别，每个组别均匀取k帧（k=1，2，3...)。用CNN提出特征，用mlp进行聚合

> 主动学习

    · 比如在标签分类等一系列任务中，将所有视频都打上标签成本太高
    · 可以用一小部分数据精标，训练模型，找出模型在剩余样本上分类难的case（信息熵最大-相当于预测每个类都很平均；top前几差异特别小的）
    · 在更新样本上进行训练，相当于帮助模型在边界处更好的优化
    · 对于主动学习来说，挑选新样本的策略是最重要的
    
> 多标签分类损失函数

    · cross engtropy的特例，对每个标签进行二分类损失预测
    · 交叉熵与最大似然估计是等价的
    · ce loss：-sum(y*log(p)),得到的是一个值
    · bce loss: 对每个类别进行一个二分类-(ylogy +(1-y)logy)，得到的是一个向量，需要平均一次ce一样

> 样本不平衡

    · 扩充数据集，多的欠采样，少的过采样
    · 小类错分加权loss
    · focal loss：-(1-p)^gamma * log(p) 当p越大，loss越小。因此错误的样本会被添加更大的权重
